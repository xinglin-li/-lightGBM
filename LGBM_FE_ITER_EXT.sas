...
%macro lgbm_run;
	...
	/*Drop previous table*/
	proc cas;
		table.droptable / name="&vf_tableOutPrefix..scored_lgb" 
			caslib="&vf_caslibOut" quiet=true;
		table.droptable / name="&vf_tableOutPrefix..lgbmStore" caslib="&vf_caslibOut" 
			quiet=true;
	...
	proc tsmodel data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n
		...
		var &targetVar  &vf_depVar &vf_indepVars _roleVar 
			%intervalFeatureVarList &EXFEATURES;
		OUTSCALAR runtime;
		REQUIRE extlang;
		PRINT outlog;
		SUBMIT;
		...
		/*The feature for the first step in forecasting horizon should also be extracted, so we assign a non-empty value for the
		dependent variable in that period*/
		rc=py.PushCodeLine("target_var['TargetVar'] = target_var['TargetVar'].fillna(method='ffill', limit=1)") ;
	...
	proc cas;
		simple.summary / table={caslib="&vf_caslibOut", 
			name="&vf_tableOutPrefix..fxInData"}, casOut={caslib="&vf_caslibOut", 
			name="&vf_tableOutPrefix..summaryOut", replace=TRUE};
	quit;
	...
	/* Step 5: Update the input table*/
	data &vf_libOut.."&vf_tableOutPrefix..fxInData"n replace;
		set &vf_libOut.."&vf_tableOutPrefix..fxInData"n(drop=&allMissingCols);
	run;
	...
	run;
		...
		proc astore;
			score data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n(where=(_roleVar 
				in (1, 2))) out=&vf_libOut.."&vf_tableOutPrefix..scored_lgb_train"n
				rstore=&vf_libOut.."&vf_tableOutPrefix..lgbmStore"n
				copyvars=(&vf_byVars &vf_timeID &targetVar &vf_depVar &vf_indepVars _roleVar 
				%intervalFeatureVarList &selectedfeatures);
		run;

		/* Concatenate both train table and forecasting table together */
		data &vf_libOut.."&vf_tableOutPrefix..scored_lgb"n;
			set &vf_libOut.."&vf_tableOutPrefix..scored_lgb_train"n &vf_libOut.."&vf_tableOutPrefix..fxInData"n(where=(_roleVar=0));
		run;

		/* The number of iterations should be the steps of forecasting */
		%do i=1 %to %eval(&vf_lead);
			%let date_idx=%sysfunc(intnx(&vf_timeIDInterval, &vf_horizonStart, 
				%eval(&i-1)));

			proc astore;
				score data=&vf_libOut.."&vf_tableOutPrefix..scored_lgb"n(where=(&vf_timeID=&date_idx)) 
			...
			proc cas;
				table.alterTable/ name="&vf_tableOutPrefix..score_next", 
					caslib="&vf_caslibOut", columns={{name="&predictVar", 
					rename="_inscalar_&predictVar"}};
			quit;

		proc tsmodel data=&vf_libOut.."&vf_tableOutPrefix..scored_lgb"n
		    inscalar=&vf_libOut.."&vf_tableOutPrefix..score_next"n
		    outarray=&vf_libOut.."&vf_tableOutPrefix..scored_lgb"n;
		    by &vf_byVars;
		    id &vf_timeID interval=&vf_timeIDInterval;
		    ...
		    REQUIRE extlang;
		    submit;
		    offset=_length_ - &vf_lead;	
		    &predictVar.[&i.+offset]=_inscalar_&predictVar;
		    
            %if %eval(&_lagYNumber>0) %then %do;

                    %do j=1 %to &_lagYNumber;
                        %if (&i+1-&j > 0) %then %do;
                            if _lagY&j[&i+1+offset]=. then do;
                                _lagY&j[&i+1+offset]=&predictVar[&i+1-&j+offset];
                            end;
                        %end;
                    %end;
		    ...
		    /* Dynamically add variables from EXFEATURES. _FEATURE1, _FEATURE2, etc. */
		    %if &i < %eval(&vf_lead) %then %do;
		        %let k=1;
		        %let varname=%scan(&selectedfeatures, &k);
		
		        %do %while(&varname ne);
		            rc=py.AddVariable(&varname, 'READONLY', 'FALSE');
		            put "AddVariable rc: " rc " varname: " &varname;
		            %let k=%eval(&k + 1);
		            %let varname=%scan(&selectedfeatures, &k);
		        %end;
		        ...
		        /* Specify shared variables */
		        rc=py.AddVariable(py_lead, "alias", "PY_LEAD");
		        rc=py.AddVariable(&predictVar, "alias", "VF_TARGET");
		        rc=py.AddVariable(&vf_timeID, "alias", "VF_ID");
		        rc=py.AddVariable(offset ,"alias", "OFFSET");
		
		        /* Push Python code lines */
		        rc=py.PushCodeLine('import numpy as np');
		        rc=py.PushCodeLine('import pandas as pd');
		        rc=py.PushCodeLine('from tsfresh import extract_features, extract_relevant_features, select_features');
		        rc=py.PushCodeLine('from tsfresh.utilities.dataframe_functions import roll_time_series,make_forecasting_frame');
		        ...
		        rc=py.PushCodeLine("target_var['TargetVar'] = target_var['TargetVar'].fillna(method='ffill', limit=1)");
		        rc=py.PushCodeLine("window_size = &_windowsize");
		        rc=py.PushCodeLine("step_size = &_windowsize");
		        rc=py.PushCodeLine("df_target = target_var.dropna()[-window_size:]");
		        rc=py.PushCodeLine("df_shift, y = make_forecasting_frame(df_target['TargetVar'], kind='value', max_timeshift=window_size, rolling_direction=step_size)");
		        rc=py.PushCodeLine("df_shift.drop(columns='kind',inplace=True)");
		        rc=py.PushCodeLine("settings = EfficientFCParameters()");
		        rc=py.PushCodeLine("extracted_features = extract_features(df_shift, column_id='id', column_sort='time', default_fc_parameters=settings)");
		        rc=py.PushCodeLine("feature_dict={}");
		        ...
		        rc=py.PushCodeLine("  feature_dict[k]=v");
		        rc=py.PushCodeLine("impute(extracted_features)");
		        rc=py.PushCodeLine("df_feature = extracted_features.droplevel(0).reset_index().rename(columns={'index':'time'})");
		
		        %let k=1;
		        %let varname=%scan(&selectedfeatures, &k);
		
		        %do %while(&varname ne);
		            rc=py.PushCodeLine('col_name = "&varname."');
		            rc=py.PushCodeLine("if not np.isnan(&varname.[OFFSET]):");
		            rc=py.PushCodeLine("    &varname.[OFFSET+&i.] = df_feature.loc[0,col_name]");
		            %let k=%eval(&k + 1);
		            %let varname=%scan(&selectedfeatures, &k);
		        %end;
