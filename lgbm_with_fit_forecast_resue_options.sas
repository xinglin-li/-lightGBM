...
%macro lgbm_fit;
    ...
    /*Drop previous table*/
    proc cas;
		table.droptable / name="&vf_tableOutPrefix..scored_lgbm_fit" caslib="&vf_caslibOut" quiet=true;
        table.droptable / name="&vf_tableOutPrefix..scored_lgbm_train_fit" caslib="&vf_caslibOut" quiet=true;
    ...
    %if %eval(&_lagYNumber=0) %then %do;
        ...
        proc astore;
            score data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n 
                out=&vf_libOut.."&vf_tableOutPrefix..scored_lgbm_fit"n
                rstore=&vf_libOut.."&vf_tableOutPrefix..lgbmStore"n
                copyvars=(&vf_byVars &vf_timeID &targetVar &vf_depVar _roleVar);
        run;
        /*promote the scored tables to ensure it can be used in forecasting task*/
        proc cas;
            table.promote / name="&vf_tableOutPrefix..scored_lgbm_fit" caslib="&vf_caslibOut";
        quit;
    ...
    %if %eval(&_lagYNumber>0) %then %do;
        ...
        proc astore;
            score data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n(where=(_roleVar in (1, 2)))
                out=&vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train_fit"n
                rstore=&vf_libOut.."&vf_tableOutPrefix..lgbmStore"n
        ...
        data &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_fit"n;
            set &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train_fit"n &vf_libOut.."&vf_tableOutPrefix..fxInData_forecasting"n;
        run;
        
        /*promote the scored tables to ensure it can be used in forecasting task*/
        proc cas;
            table.promote / name="&vf_tableOutPrefix..scored_lgbm_train_fit" caslib="&vf_caslibOut";
            table.promote / name="&vf_tableOutPrefix..scored_lgbm_fit" caslib="&vf_caslibOut";
        quit;
        ...
        quit;
    %end;
    /*Create a table stores all lag specifications of trained model */
	...
	data &vf_libOut.."&vf_outFor"n;
		set &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_fit"n;
		actual=&targetVar;
		predict=&predictVar;
...
%macro lgbm_forecast;
    ...
    proc cas;
        ...
        table.droptable / name="&vf_tableOutPrefix..scored_lgbm_train" caslib="&vf_caslibOut" quiet=true;
        table.droptable / name="&vf_tableOutPrefix..scored_lgbm_new" caslib="&vf_caslibOut" quiet=true;
        table.droptable / name="&vf_tableOutPrefix..scored_lgbm_train_new" caslib="&vf_caslibOut" quiet=true;
		table.droptable / name="&vf_outFor" caslib="&vf_caslibOut" quiet=true;
    quit;
    

    %if %eval(&_lagYNumber=0) %then %do;
        /* Determine whether to reuse the predictions of both training and forecasting from the previous fitted process */
        %if %upcase(&_reusePrevPrediction) eq YES %then %do;
            /* Step 1: Determine the maximum date in the previous scored training dataset */
            data _null_;
                set &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_fit"n(where=(_roleVar in (1, 2))) end=last;
                retain max_date;
                if _n_ = 1 then max_date = &vf_timeID; /* Initialize on the first iteration */
                if &vf_timeID > max_date then max_date = &vf_timeID; /* Update max_date if a higher value is found */
                if last then call symputx('last_date', max_date); /* Store max_date in a macro variable */
            run;
            /* Step 2: Score the new data where the time ID is greater than the max date from previous scoring */
            proc astore;
                score data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n(where=(&vf_timeID > &last_date))
                    out=&vf_libOut.."&vf_tableOutPrefix..scored_lgbm_new"n
                    rstore=&vf_libOut.."&vf_tableOutPrefix..lgbmStore"n
                    copyvars=(&vf_byVars &vf_timeID &targetVar &vf_depVar);
            run;
            /* Step 3: Concatenate the previous scored data and the newly scored data */
            data &vf_libOut.."&vf_tableOutPrefix..scored_lgbm"n;
                set &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_fit"n(where=(_roleVar in (1, 2)))
                    &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_new"n;
            run;
        %end;
        %else %do;
            /* If no reuse, score the entire dataset from scratch */
            proc astore;
                score data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n 
    ...
    %if %eval(&_lagYNumber>0) %then %do;
        %if %upcase(&_reusePrevPrediction) eq YES %then %do;
            /* Step 1: Determine the maximum date in the previous scored training dataset */
            data _null_;
                set &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train_fit"n end=last;
                retain max_date;
                if _n_ = 1 then max_date = &vf_timeID; /* Initialize on the first iteration */
                if &vf_timeID > max_date then max_date = &vf_timeID; /* Update max_date if a higher value is found */
                if last then call symputx('last_date', max_date); /* Store max_date in a macro variable */
            run;
            /* Step 2: Score the new data where the time ID is greater than the max date from previous scoring in training and validation sample */
            proc astore;
                score data=&vf_libOut.."&vf_tableOutPrefix..fxInData"n
                    (where=(_roleVar in (1, 2) and &vf_timeID > &last_date))
                    out=&vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train_new"n
                    rstore=&vf_libOut.."&vf_tableOutPrefix..lgbmStore"n
                    copyvars=(&vf_byVars &vf_timeID &targetVar &vf_depVar &vf_indepVars %intervalFeatureVarList);
            run;

            /* Step 3: Concatenate the previous scored data and the newly scored data in training and validation sample */
            data &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train"n;
                set &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train_fit"n
                    &vf_libOut.."&vf_tableOutPrefix..scored_lgbm_train_new"n;
            run;
        %end;
        %else %do;
            /* Score both the training and validation data */
